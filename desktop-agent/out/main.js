...
function appendOfflineLog(filename, payload) {
    try {
        ensureCacheDirs();
        const filePath = path_1.default.join(getOfflineCacheDir(), filename);
        let logs = [];
...
async function flushOfflineCache() {
    ...
    try {
        ...
        const usagePath = path_1.default.join(getOfflineCacheDir(), 'usage-logs.json');
        if (fs_1.default.existsSync(usagePath)) {
            ...
            if (logs.length > 0) {
                ...
                for (const log of logs) {
                    try {
                        await axios_1.default.post(`${BACKEND_URL}/api/monitoring/usage-log`, log, { headers: { Authorization: `Bearer ${sessionToken}` } });
        ...
        const activityPath = path_1.default.join(getOfflineCacheDir(), 'activity-logs.json');
        if (fs_1.default.existsSync(activityPath)) {
            ...
            if (logs.length > 0) {
                ...
                for (const log of logs) {
                    try {
                        await axios_1.default.post(`${BACKEND_URL}/api/monitoring/activity`, log, { headers: { Authorization: `Bearer ${sessionToken}` } });
            ...
            if (ssFiles.length > 0) {
                ...
                for (const file of ssFiles) {
                    ...
                    try {
                        ...
                        await axios_1.default.post(`${BACKEND_URL}/api/monitoring/screenshot`, {
                            image: content.image,
...
function createSplashWindow() {
    ...
    splashWindow.loadFile(path_1.default.join(__dirname, '../splash.html'));
...
electron_1.app.whenReady().then(() => {
    ...
    // Set up System Tray
    tray = new electron_1.Tray(path_1.default.join(__dirname, '../win-icon.ico'));
...
electron_1.app.on('before-quit', (event) => {
    if (sessionToken && !isCheckingOut) {
        ...
        axios_1.default.post(`${BACKEND_URL}/api/attendance/checkout`, {}, {
            headers: { Authorization: `Bearer ${sessionToken}` },
            timeout: 3000
        }).then(() => {
...
electron_1.ipcMain.on('screenshot:cache', (event, { image }) => {
    try {
        ...
        const filePath = path_1.default.join(getOfflineScreenshotsDir(), `screenshot_${timestamp}.json`);
        const payload = {
            image,
            timestamp
        };
...
function startTracking() {
    ...
    if (sleepBlockerId === null) {
        sleepBlockerId = electron_1.powerSaveBlocker.start('prevent-display-sleep');
    ...
    if (process.platform === 'win32') {
        try {
            ...
            const pathExists = fs_1.default.existsSync(monitorPath);
            try {
                fs_1.default.appendFileSync(debugLogPath, `[${new Date().toISOString()}] Desktop Agent: Spawning background activity monitor sidecar. monitorPath="${monitorPath}" exists=${pathExists}\n`);
            }
            catch (e) { }
            console.log('Desktop Agent: Spawning background activity monitor sidecar...');
            activityProcess = (0, child_process_1.spawn)('powershell', [
                '-NoProfile',
                '-ExecutionPolicy',
                'Bypass',
                '-File',
            ...
            try {
                fs_1.default.appendFileSync(debugLogPath, `[${new Date().toISOString()}] Desktop Agent: Spawned powershell. PID=${activityProcess.pid}\n`);
            }
            catch (e) { }
            activityProcess.stdout.on('data', (data) => {
                ...
                try {
                    fs_1.default.appendFileSync(debugLogPath, `[${new Date().toISOString()}] Activity Monitor Stdout - Raw length: ${rawText.length}, Clean: "${cleanText.trim()}"\n`);
                }
                catch (e) { }
                ...
                for (let line of outputLines) {
                    ...
                    if (line.startsWith('KEYS:')) {
                        ...
                        try {
                            fs_1.default.appendFileSync(debugLogPath, `[${new Date().toISOString()}] Parsed Activity - Keys: ${keys}, Clicks: ${clicks}. Cumulative Active in tick: ${activeSecondsInTick}s, Idle: ${idleSecondsInTick}s\n`);
                        }
                        catch (e) { }
            ...
            activityProcess.stderr.on('data', (data) => {
                ...
                try {
                    fs_1.default.appendFileSync(debugLogPath, `[${new Date().toISOString()}] Activity Monitor STDERR: "${errText.trim()}"\n`);
                }
                catch (e) { }
            });
            activityProcess.on('close', (code) => {
                ...
                try {
                    fs_1.default.appendFileSync(debugLogPath, `[${new Date().toISOString()}] Activity Monitor Process EXITED with code: ${code}\n`);
                }
                catch (e) { }
            });
            activityProcess.on('error', (err) => {
                ...
                try {
                    fs_1.default.appendFileSync(debugLogPath, `[${new Date().toISOString()}] Activity Monitor Process ERROR: ${err.message}\n`);
                }
                catch (e) { }
            });
        }
            ...
            try {
                fs_1.default.appendFileSync(debugLogPath, `[${new Date().toISOString()}] Failed to start background activity monitor: ${err.message}\n`);
            }
            catch (e) { }
        }
    }
...
function stopTracking() {
    ...
    if (sleepBlockerId !== null) {
        if (electron_1.powerSaveBlocker.isStarted(sleepBlockerId)) {
            electron_1.powerSaveBlocker.stop(sleepBlockerId);
...
async function sendHeartbeat() {
    if (!sessionToken)
        return;
    try {
        const response = await axios_1.default.post(`${BACKEND_URL}/api/attendance/heartbeat`, {}, { headers: { Authorization: `Bearer ${sessionToken}` } });
...
async function flushActivityTelemetry() {
    ...
    try {
        await axios_1.default.post(`${BACKEND_URL}/api/monitoring/activity`, {
...
function handleActiveWindowOutput(stdout) {
    ...
    try {
        fs_1.default.appendFileSync(logPath, `[${new Date().toISOString()}] Captured output: "${output}"\n`);
    }
    catch (e) { }
...
async function flushUsageBuffer() {
    ...
    for (const appName of Object.keys(logsToFlush)) {
        ...
        try {
            await axios_1.default.post(`${BACKEND_URL}/api/monitoring/usage-log`, {
                appName,
                windowTitle: log.windowTitle,
                type: log.type,
                durationMinutes
