...
function EmployeeMonitoring() {
  ...
  // --- Pagination ---
  const [appUsagePage, setAppUsagePage] = useState(1);
        ...
        <Grid size={{ xs: 12 }}>
          <Card sx={{ borderRadius: 3 }}>
            <CardContent>
              ...
              <Divider sx={{ mb: 2 }} />
              {latestCheckin && Array.isArray(latestCheckin.breakHistory || latestCheckin.breaks) && (latestCheckin.breakHistory || latestCheckin.breaks).length > 0 ? (
                <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap' }}>
                  {(latestCheckin.breakHistory || latestCheckin.breaks).map((b: any, i: number) => {
                    ...
                    return (
                      <Chip 
                        key={i} 
                        label={`${b?.breakType || b?.break_type || b?.type || 'Break'}: ${diffMins}m ${b?.note ? `"${b.note}"` : ''}`} 
                        variant="outlined"
                        color="secondary"
                      />
                    );
                ...
                <Box>
                  ...
                  <TableContainer component={Paper} variant="outlined" sx={{ border: 'none' }}>
                    <Table>
                      ...
                      <TableBody>
                        {appUsage.slice((appUsagePage - 1) * rowsPerPage, appUsagePage * rowsPerPage).map((item) => (
                  ...
                  {/* App Usage Pagination */}
                  {appUsage.length > rowsPerPage && (
                    <Box sx={{ display: 'flex', justifyContent: 'center', mt: 3 }}>
                      <Pagination
                        count={Math.ceil(appUsage.length / rowsPerPage)}
                        page={appUsagePage}
                        onChange={(e, value) => setAppUsagePage(value)}
                        color="primary"
                        shape="rounded"
                      />
                    </Box>
        ...
        <Grid size={{ xs: 12 }}>
          <Card sx={{ borderRadius: 3 }}>
            <CardContent>
              ...
              {/* Activity Pagination */}
              {activity.length > rowsPerPage && (
                <Box sx={{ display: 'flex', justifyContent: 'center', mt: 3 }}>
                  <Pagination
                    count={Math.ceil(activity.length / rowsPerPage)}
                    page={activityPage}
