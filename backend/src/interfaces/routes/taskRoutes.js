...
router.delete('/:id', authenticate, authorize(['Manager', 'SuperAdmin']), async (req, res) => {
  try {
    const { data: task, error: fetchErr } = await supabase
      .from('tasks')
      .select('*')
...
router.post('/:id/submit', authenticate, uploadAttachment.array('files', 10), async (req, res) => {
  try {
    const { data: task, error: fetchErr } = await supabase
      .from('tasks')
      .select('*')
